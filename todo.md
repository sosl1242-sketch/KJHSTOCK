...
## 12시간 주기 Heartbeat 스케줄 추가 (2026-05-13 #5)
- [x] 캐시 동기화 라우트 추가 (server/scheduled.ts)
- [x] 라우트 등록 (server/_core/index.ts)
- [x] Heartbeat 스케줄 관리 함수 작성 (server/cacheAutoRefresh.ts)
- [x] TypeScript 검사 통과 (36개 테스트 모두 통과)
- [x] 관리자 패널에서 스케줄 수동 활성화 기능 추가 (routers.ts에 3개 라우터 추가)
...
- [x] TypeScript 검사 통과 (36개 테스트 모두 통과)
- [x] 크립토 초기 캠닜 데이터 로드: 서버 시작 시 동기적 로드 + 12시간 주기 Heartbeat + 관리자 수동 로드
